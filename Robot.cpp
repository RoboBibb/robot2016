...
void Robot::TeleopPeriodic(){
	...
	if ((stick.GetRawButton(2) && shooterInLim.Get()) || stick.GetRawButton(3))
		setMotorDirection(inAndOut1, stick, 3, 2); //set intake/fire
	else inAndOut1.SetSpeed(0);

    //this is because they want the wire colors to match on the motors
	if ((stick.GetRawButton(3) && shooterInLim.Get()) || stick.GetRawButton(2))
		setMotorDirection(inAndOut2, stick, 2, 3); //set intake/fire
